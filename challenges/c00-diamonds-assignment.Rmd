...
Since both better cut and better clarity seem to be correlated with price, I wonder if cut and clarity are correlated. It looks like most of the best clarity diamonds also have ideal cut, but poor clarity diamonds are distributed fairly evenly across different cuts. So they are correlated for the best diamonds, but not for worse diamonds.

```{r cut vs clarity}
ggplot(diamonds, aes(cut, clarity)) +
  geom_jitter(aes(color = cut), size = 0.5)
```
How does price vary for the best cut diamonds? For the best cut diamonds, price increases with carat, and the higher clarity diamonds fetch higher prices for a given carat.
...
diamonds %>%
  ...
  geom_point(mapping = aes(x = carat, y = price, color = clarity))
```
Carat seems to be the variable most strongly correlated with price. I wonder which secondary variable is more important to price, cut or clarity, but I'm not sure yet how to answer this question.
