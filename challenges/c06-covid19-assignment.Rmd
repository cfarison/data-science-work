...
#Where did we learn how to load .csv's?
#My table was not called B01003, is that fine? It was not fine. I found B01003 by plugging it into the URL, not sure why mine was different.
#I got something, but I can't tell if it's right. The data looks weird.
#You never said to use 5 year, but  I could see from the filename.
#Help us put data in right column name?
filename_census = "./data/ACSDT5Y2018.B01003_data_with_overlays_2020-07-24T214459.csv"
df_pop <- 
  read_csv(filename_census, col_names = c("id", "Geographic Area Name", "Estimate!!Total", "Margin of Error")) %>%
  .[-c(1:2),]
```
...
# Maybe tell them to read the readme? That has the link.
...
#Looks like my .csv is not the data
...
df_q3 <- 
  ...
  select(-c("prefix", "Margin of Error"))
df_q3
```
...
## Check known county
#You didn't specify our column names
...
#Again, didn't specify column names
df_data <-
  df_q4 %>%
  select(
    date,
...
# I feel pretty confident, but this says it's wrong.
assertthat::assert_that(
              abs(df_normalized %>%
               filter(
