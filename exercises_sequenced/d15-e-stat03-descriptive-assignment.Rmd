...
Top means: F9, EV, YV, FL, WN
Top medians: FL, WN, F9, UA, VX
- Which carriers differ in the two lists? The 3 that are on both lists are F9, FL, and WN.
- What do negative values of `dep_delay` mean? Early deepartures/arrivals.
...
sd: HA, F9, FL, YV, EV
IQR: EV, YV, 9E, F9, FL
- Which carriers differ in the two lists? F9, FL, EV, and YV are in both lists. HA is in sd, 9E is in IQR.
- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
HA has a difference of 68 between sd and IQR. This might mean it has a high outlier (or multiple).
...
- For what values of `slope` is the correlation positive? positive values of slope 
- For what values of `slope` is the correlation negative? negative values of slope
...
- What do you predict the correlations to be? I predicted spearman to be 1, and pearson to be less than 1.
- What are the actual correlations? Pearson is 0.85, Spearman is 1.
