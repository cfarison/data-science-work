...
The expectation is *linear*, that is $\mathbb{E}[aX + c] = a \mathbb{E}[X] + c$. We can use this fact to compute the mean of simply transformed random variables.
...
Variance satisfies the property $$\mathbb{V}[aX + c] = a^2 \mathbb{V}[X].$$

__q2__ Compute the variance of $2 Z + 3$, where $Z$ is a standard normal.

```{r q2-task}
## TASK: Compute the variance of 2 Z + 3
V_q2 <- 
  2^2 * 1
```
...
__q3__ Compute the mean and variance of $(X - 1) / 2$, where $\mathbb{E}[X] = 1$ and $\mathbb{V}[X] = 4$.

```{r q3-task}
## TASK: Compute the mean and variance
#Mean of x is 1, variance of x is 4
E_q3 <- (1-1)/2
V_q3 <- (1/2)^2*4
```
...
where $\text{Cov}[X, Y] = \mathbb{E}[(X - \mathbb{E}[X])(Y - \mathbb{E}[Y])]$ is the *covariance* between $X$ and $Y$. If $X, Y$ are independent, then the covariance between them is zero.

Using this expression, we can prove that the standard error of the sample mean $\overline{X}$ is $\sigma / \sqrt{n}$.

__q5__ (Bonus) Use the identity above to prove that $\mathbb{V}[\overline{X}] = \sigma^2 / n$, where $\overline{X} = \frac{1}{n}\sum_{i=1}^n X_i$, $\sigma^2 = \mathbb{V}[X]$, and the $X_i$ are mutually independent.
